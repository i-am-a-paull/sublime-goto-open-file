...
class ViewSelector(object):
  ...
  def __init__(self, window, active_group):
    ...
    self.items = sorted([[self.__get_display_name(view), self.__get_path(view)] for view in self.views])

  def select(self, index):
    if index != -1:
      self.window.focus_view(self.__get_view_by_path(self.items[index][1]))
